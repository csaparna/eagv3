...
if os.getenv("GLC_ENV", "").lower() == "production":
    docs_url = None
    openapi_url = None
...
ALLOWLIST_PATHS = [
  ...
  #  re.compile(r"^/v1/channels/[^/]+/webhook$"),
]

@app.middleware("http")
async def global_auth_middleware(request: Request, call_next):
    path = request.url.path
    if any(pattern.match(path) for pattern in ALLOWLIST_PATHS):
        return await call_next(request)
        
    expected_token = get_or_create_install_token()
