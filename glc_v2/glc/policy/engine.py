...
class PolicyEngine:
    ...
    def evaluate(self, tool_call: dict[str, Any], context: dict[str, Any]) -> PolicyVerdict:
        ...
        if first_match is not None:
            i, r = first_match
            # Move 3: Issue a scoped credential for the tool call instead of relying on global env keys
            issued = f"scoped_token_for_{tool}" if r.action == "allow" else None
            return PolicyVerdict(
                action=r.action, reason=r.reason or f"matched rule #{i}", matched_rule_index=i, issued_credential=issued
            )
