...
2. log_paper_record(title: str, authors: str, doi: str, tldr: str, citation_count: int, csv_path: str = "papers.csv") -> str
   Add a paper's details to a CSV file as a separate row. Call this for each paper you want to log.
   Examples: log_paper_record("Attention is all you need", "Ashish Vaswani, Noam Shazeer...", "10.48550/arXiv.1706.03762", "A new network architecture based on attention mechanisms.", 10000, "papers.csv")
...
def search_semantic_scholar(query: str, offset: int = 0, limit: int = 1) -> str:
    """Search Semantic Scholar for a paper and extract authors, title, tldr, citationCount, and DOI."""
    import requests
    import json
...
def log_paper_record(title: str, authors: str, doi: str, tldr: str, citation_count: int, year: int, csv_path: str = "papers.csv") -> str:
    """Add a paper's details to a CSV file."""
    import csv
    import os
    import json
    try:
        ...
        with open(csv_path, mode='a', newline='', encoding='utf-8') as file:
            ...
            if not file_exists:
                writer.writerow(['Title', 'Authors', 'DOI', 'TLDR', 'Citation Count'])
            writer.writerow([title, authors, doi, tldr, citation_count])
